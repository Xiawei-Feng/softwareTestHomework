...
public class AdminUserControllerModified {
...
//        要登录的用户的角色应该是管理员
        ServerResponse response = userService.login(phoneNumber,
...
//        把角色的信息存储到session中
    ...
    @ResponseBody
    public ServerResponse list(HttpSession session, @RequestParam(
            value = "role", required = false) Integer userType,
...
//      管理员已经登录，下面执行查询业务
        return userService.getUserList(userType, userStatus,
                pageNumber, pageSize);
    }
    ...
    @ResponseBody
    public ServerResponse updateUserStatus(HttpSession session, @RequestParam(value = "userId",
            required = true) Integer userId, @RequestParam(value = "status",
                                                   required = true) Integer userStatus) {
    ...
    @ResponseBody
    public ServerResponse resetPassword(HttpSession session, String oldPassword,
                                        String newPassword) {
        User currentUser = (User) session.getAttribute(Role.ADMIN.getDesc());
        if (currentUser == null) {
            return ServerResponse.createByErrorCodeMessage(ResponseCode.NEED_LOGIN.getCode(),
        ...
        return userService.resetPassword(currentUser.getUserId(),
                MD5Utils.MD5EncodeUtf8(oldPassword),
                MD5Utils.MD5EncodeUtf8(newPassword));
    }

    @RequestMapping(value = "get_audit_material")
    @ResponseBody
    public ServerResponse getAuditMaterial(String sellerId,
                                           HttpSession session) {
        User currentUser = (User) session.getAttribute(Role.ADMIN.getDesc());
        if (currentUser == null) {
            return ServerResponse.createByErrorCodeMessage(ResponseCode.NEED_LOGIN.getCode(),

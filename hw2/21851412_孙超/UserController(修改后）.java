...
//        用户已经登录，返回个人信息
    ...
    public ServerResponse resetPassword(String authToken, String oldPassword,
                                        String newPassword)
    {
        if (StringUtils.isBlank(authToken) || StringUtils.isBlank(oldPassword)
...
//        用户已经登录，并且要修改的信息有效，不为空。调用业务层来修改密码
        User user = JsonUtil.str2Object(userString, User.class);
        return userService.resetPassword(user.getUserId(), MD5Utils.MD5EncodeUtf8(oldPassword),
    ...
    {
        if (StringUtils.isBlank(phoneNumber) || StringUtils.isBlank(newPassword)
